...
@api_router.get("/")
async def root():
    return {"message": "Nishchay Bhardwaj portfolio API"}
...
async def send_contact_message(request: ContactRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="We could not send your message right now") from exc
...
app.add_middleware(
    ...
    allow_origins=os.environ.get('CORS_ORIGINS', 'https://portfolio-ekc5.vercel.app,https://dev-nishchay.preview.emergentagent.com,http://localhost:3000').split(','),
    allow_methods=["*"],
    allow_headers=["*"],
)
